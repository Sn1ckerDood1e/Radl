...
must_haves:
  ...
  key_links: []
  notes:
    - "SC-1 (JWT claims): Already satisfied by Phase 12 - supabase/migrations/00006_facility_access_token_hook.sql injects facility_id, club_id, user_roles into JWT claims via custom_access_token_hook"
    - "SC-4 (RLS session variables): Satisfied by Phase 12's architecture - RLS helper functions (get_current_facility_id(), get_current_club_id() in 00005_facility_rls_helpers.sql) read directly from JWT claims via current_setting('request.jwt.claims', true), NOT session variables. This is connection-pooling safe and equivalent to session variables for RLS purposes."
---
...
**Architecture notes for verification:**
- SC-1 (JWT claims): Phase 12's custom_access_token_hook (00006_facility_access_token_hook.sql) already injects facility_id, club_id, and user_roles into JWT claims
- SC-4 (RLS): Phase 12's RLS helper functions (00005_facility_rls_helpers.sql) read from JWT claims via current_setting('request.jwt.claims', true) - this is connection-pooling safe and equivalent to session variables
  </what-built>
  <how-to-verify>
...
5. Test invalid cookie recovery:
   ...
   - Manually change radl_current_club to invalid UUID
   - Refresh page
