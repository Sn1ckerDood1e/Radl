...
Verify AthleteCard can be rendered in DragOverlay without hooks error:
1. AthleteCard has zero imports from @dnd-kit/*
2. AthleteCard does not call useSortable, useDroppable, or any other hooks from dnd-kit
3. Confirm AthleteCard is a pure presentation component (only receives props, no internal hook calls that would fail in DragOverlay context)
  </verify>
