...
must_haves:
  truths:
    ...
    - "System can map IDP groups to Radl roles"
...
Purpose: Enable facility admins to configure SAML SSO and map identity provider groups to Radl roles. Per CONTEXT.md: SSO is configured at facility level, users without matching roles default to ATHLETE.
...
- `mapSsoRoles` maps IDP groups to Radl roles per configuration
