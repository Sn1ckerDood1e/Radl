...
export function SsoConfigForm() {
  ...
  const addMapping = () => {
    setRoleMappings([...roleMappings, { idpValue: '', radlRoles: [] }]);
  };
  ...
  const toggleMappingRole = (index: number, role: string) => {
    const current = roleMappings[index].radlRoles;
    const updated = current.includes(role)
      ? current.filter(r => r !== role)
      : [...current, role];
    updateMapping(index, 'radlRoles', updated);
  };
      ...
      <div className="p-4 bg-card rounded-lg border space-y-4">
        ...
        {!config?.ssoProviderId && enabled && (
          <div className="p-3 bg-amber-50 dark:bg-amber-950 text-amber-800 dark:text-amber-200 rounded-lg text-sm">
            ...
            Contact Radl support for assisted setup.
          </div>
        )}
      </div>
      ...
      <div className="p-4 bg-card rounded-lg border space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Map identity provider groups to Radl roles.
            </p>
          </div>
                  ...
                  {AVAILABLE_ROLES.map((role) => (
                    <button
                      ...
                      className={`px-2 py-1 text-xs rounded-full border ${
                        mapping.radlRoles.includes(role)
    ...
    <div className="container max-w-2xl py-8 space-y-8">
      ...
      <div className="p-4 bg-muted rounded-lg text-sm space-y-2">
        ...
        <p className="text-muted-foreground">
          ...
          Google Workspace, etc.) and the Supabase CLI. Contact Radl support for
          assisted setup.
        </p>
        <p className="text-muted-foreground">
