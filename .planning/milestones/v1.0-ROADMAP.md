...
|----------|-----------|---------|
| Security first | Existing JWT gaps are active vulnerabilities | Good - solid foundation |
| Scheduling before Lineups | Data model dependency | Good - clean architecture |
| Offline before Regatta | Regatta is primary offline use case | Good - infrastructure reused |
| PWA over native | Service workers handle push + offline | Good - works well |
| Lineup-first assignment | Coaches think in people, boats are constraints | Good - natural flow |
| Per-team RC keys | Each team connects own account | Good - proper isolation |
| dnd-kit for drag-drop | Modern, accessible, well-maintained | Good - smooth UX |
| Serwist for service worker | Production-tested caching strategies | Good - reliable offline |

**Issues Resolved:**
- Gap closure 02-07/02-08: Template UI and equipment visibility
- Gap closure 05-08: Offline regatta wiring

**Tech Debt Deferred to v2.0:**
- Regatta Central import UI (APIs ready, manual entry workaround)
- Equipment usage display UI
