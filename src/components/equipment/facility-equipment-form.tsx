...
interface FacilityEquipmentFormProps {
  facilitySlug: string;
  facilityId: string;
}
...
/**
 * Form for creating new facility-owned equipment.
 * Handles all equipment types with conditional rendering for shell-specific fields.
 *
 * @param facilitySlug - Current facility's slug for navigation after successful creation
 * @param facilityId - Facility ID for API request
 */
export function FacilityEquipmentForm({ facilitySlug, facilityId }: FacilityEquipmentFormProps) {
  ...
  const onSubmit = async (data: CreateEquipmentFormInput) => {
    ...
    try {
      ...
      const response = await fetch(`/api/facility/${facilityId}/equipment`, {
        method: 'POST',
